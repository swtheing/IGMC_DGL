...
class MovieLens_bak(object):
    ...
    def __init__(self, name, device, mix_cpu_gpu=False,
        ...
        if self._name == 'ml-100k':
            ...
            # @@
            print(type(self.all_train_rating_info))
        ...
        print(self.global_movie_id_map)
        exit(0)
...
def load_matlab_file(self, path_file, name_field):
    """
    load '.mat' files
    inputs:
...
class DataSetLoader(object):
    ...
    def __init__(self, name, device, mix_cpu_gpu=False,
        ...
        # if self._name == "yahoo_music":
        #     possible_rate = []
        #     for i in range(100):  # 0-99
        #         possible_rate.append(i)
        #     self.possible_rating_values = possible_rate
        # else:
        # self.possible_rating_values = np.unique(self.all_train_rating_info["rating"].values)
        ...
        # for i in range(3000):
        #     self.global_user_id_map[i] = i
        # for i in range(3000):
        #     self.global_movie_id_map[i] = i
        #     self.global_user_id_map = {ele: i for i, ele in enumerate(self.user_info['id'])}
        #     self.global_movie_id_map = {ele: i for i, ele in enumerate(self.movie_info['id'])}
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        # print(list(set(rating_values)))
        # la = list(set(rating_values))
        # for i in range(len(self.possible_rating_values)):
        #     print("%s %s" % (self.possible_rating_values[i], la[i]))
        # print(len(rating_values))
        # print(len(set(rating_values)))
        # for x in rating_values:
        #     print("v: %s" % x)
    ...
    def load_data_monti(self, dataset, testing=False, rating_map=None, post_rating_map=None, own = False):
        """
...
if __name__ == '__main__':
    ...
    # MovieLens_bak("ml-100k", device=th.device('cpu'), symm=True)
    
    rating_map = None
    testing = True
    post_rating_map = None
    data_name = "flixster"
    daload = DataSetLoader(data_name)
    (
        u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices,
        val_labels, val_u_indices, val_v_indices, test_labels, test_u_indices, 
        test_v_indices, class_values, num_user, num_item
    ) = daload.load_data_monti(data_name, testing, rating_map, post_rating_map)
    assert len(train_labels) == len(train_u_indices)
    assert len(train_labels) == len(train_v_indices)
    ...
    # init_data = pd.DataFrame(data_dict, index=[])
    # indexs = []

    # for i in range(train_size):
    #     data_dict["user_id"].append(int(train_u_indices[i]))
    #     data_dict["movie_id"].append(int(train_v_indices[i]))
    #     data_dict["ratings"].append(float(train_labels[i]))
    #     data_dict["timestamp"].append(int(999))
    #     indexs.append(i)

    # print(init_data)
    # train_data_set = pd.DataFrame(data_dict, index=indexs)
    # train_data_set = daload.trans_loader(train_labels, train_u_indices, train_v_indices)
    # print(train_data_set)
    test_data_set = daload.trans_loader(test_labels, test_u_indices, test_v_indices)
    print(test_data_set)
    print(test_data_set["user_id"])
    global_user_map = {}
    global_movie_map = {}
    for i in range(num_user):
        global_user_map[int(i+1)] = i
    for i in range(num_item):
        global_movie_map[int(i+1)] = i

    print(global_user_map)
    print(global_movie_map)
